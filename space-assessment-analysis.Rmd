...
util_data <- utilization_tbl %>%
  ...
  filter(DEPARTMENT %like% "340 E 17TH ST FAMILY PLANNING") %>%
  collect() %>% 
  left_join(access_raw_tbl %>% 
...
depts <- util_data %>%
  group_by(DEPARTMENT, VISIT_METHOD) %>%
  summarise(total = n()) %>%
...
dept_visits_types <- util_data %>%
  group_by(DEPARTMENT, VISIT_TYPE, PRC_NAME) %>%
  summarise(total = n())
...
####### Analysis Criteria ########
# 5 E 98th St --------------------
visits_exclude <- "FLUROSCOPY|FLUOROSCOPIC|ULTRASOUND|INFUSION|PHOTOTHERAPY"
visit_types_exclude <- "NA"
depts_exclude <- "NA"
total_rooms <- 272 # Including 265 exam + 7 procedure 


# 234 85th St --------------------
visits_exclude <- "AUDIO TEST"
visit_types_exclude <- c("")
depts_exclude <- c("234 E 85TH CARD ENDO","234 E 85TH CARD REHAB","234 E 85TH CARDIOLOGY") # Moved out in Q1 2023
total_rooms <- 85 # Including 75 exam + 10 procedure
...
depts_exclude <- c(
  # "440 W 114 CARDIO REMOTE DEVICE",
                    "440 W 114 IAM",
                    "440 W 114 INFUSION",
                    "440 W 114 INFUSION CENTER",
                    "440 W 114 LAB CLINIC",
                    "440 W 114TH IAM DENTAL")
total_rooms <- 95 # Including 110 exam + 1 procedure

# 340 E 17th St Family Planning --------
visits_exclude <- c("")
# visit_types_exclude <- "TREATMENT VISIT"
depts_exclude <- c("")
total_rooms <- 3 # Including 110 exam + 1 procedure
...
util_data <- util_data %>%
  ...
  filter(APPT_DATE_YEAR < as.Date("2023-11-01")) %>%
...
require(openxlsx)
write.xlsx(visits_provider, "440 West 114th St Visits by Provider Updated.xlsx")
